<script lang="ts">
  ...
  async function loadInstalledItems() {
    ...
    try {
      const mods = await invoke<{ filename: string }[]>("get_installed_mods", { instanceName: selectedInstance.name })
    ...
    <div class="{hideToolbar ? '' : 'lg:col-span-2'} space-y-3 overflow-y-auto pr-2">
      {#each hits as item (item.id)}
        <div
          ...
          onclick={() => { if (onViewProjectDetail) onViewProjectDetail("curseforge", String(item.id), item.slug, "mod", item.authors[0]?.name); handleItemSelect(item); }}
        >
      ...
      <div class="bg-[var(--bg-tertiary)] rounded-md p-3 sticky top-0 self-start">
        ...
        <div class="pt-1">
          ...
          {:else if itemFiles.length === 0}
            <p class="text-sm text-[#3a3f4b] text-center py-3">No files available</p>
          {:else}
