<script lang="ts">
  ...
  const CLASS_ID = 12
  const SEARCH_PLACEHOLDER = "Search resource packs..."
    ...
    <div class="flex-1 min-h-0 {hideToolbar ? '' : 'grid grid-cols-1 lg:grid-cols-3 gap-2'}">
      <div class="{hideToolbar ? '' : 'lg:col-span-2'} space-y-3 overflow-y-auto pr-2">
        {#each hits as item (item.id)}
          ...
          onclick={() => { if (onViewProjectDetail) onViewProjectDetail("curseforge", String(item.id), item.slug, "resourcepack", item.authors[0]?.name); handleItemSelect(item); }}
        >
            ...
            {#if item.logo?.thumbnailUrl}
              ...
              {:else}
                <div class="w-24 h-24 bg-gradient-to-br from-[#8b5cf6]/10 to-[#a78bfa]/10 flex items-center justify-center flex-shrink-0 rounded m-2">
                  <Package size={48} class="text-[#8b5cf6]" />
                </div>
              {/if}
      ...
      {#if !hideToolbar && selectedItem}
        <div class="bg-[var(--bg-tertiary)] rounded-md p-3 sticky top-0 self-start">
          ...
          <div class="pt-1">
            ...
            {#if isLoadingFiles}
              <div class="text-center py-6"><Loader2 size={20} class="animate-spin text-[#8b5cf6] mx-auto" /></div>
